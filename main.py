...
trash_img = Image.open("lib/source/trash-can.png").resize((15, 15))
...
def delete_file():
    ...
    for item_id in itens_selecionados:
        ...
        shortname = values[1] if values else None

        if shortname and shortname in data:
            del data[shortname]
...
trash_btn = ttk.Button(tab1, text="Sortering",
                       image=trash_tk, command=delete_file)
trash_btn.place(x=805, y=55)
...
trash_btn.bind("<ButtonRelease-1>", remove_focus)
...
def add_element(name, ext, path):
    ...
    element.grid(row=current_row, column=current_col, padx=20, pady=15)

    url = get_icon(ext)
    image = ImageTk.PhotoImage(Image.open(url).resize((100, 100)))

    lbl = Label(element, image=image)
    lbl.image = image
    lbl.pack(pady=2, side=TOP)

    if len(name) > 10:
        name = name[:9] + "..."

    text = Label(element, text=name, background="gray")
...
delete_tool = ttk.Button(
    tab1, width=4, style="Accent.TButton", image=trash_tk, compound="center")
